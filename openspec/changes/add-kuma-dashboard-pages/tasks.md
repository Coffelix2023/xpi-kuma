...
- [ ] 10.2 人工核对四个页面在英文下的排版不溢出（按钮、表头、空态文案）；验证方式是在浏览器里逐页切换语言并确认无截断或换行错位。**未完成**：本项需要真实浏览器渲染，当前会话无法执行；已做的准备是英文文案保持短句、表格说明列 `.kuma-note` 允许换行、按钮沿用自适应宽度。留待 11.3 的 `pi -e ./src/index.ts` 冒烟时逐页切换语言核对。
