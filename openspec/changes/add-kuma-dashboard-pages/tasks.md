...
- [x] 8.1 实现只读 `inspectConfig(cwd)`：不调用会创建模板的 `loadConfig()`，用 `existsSync` 判断存在性，存在才解析，捕获 `ConfigError` 并原样返回 `message` 与 `line`；验证方式是单测断言「文件缺失时不创建文件」与「解析失败返回行号」。结论：实现为 `src/diagnostics/inspect.ts` 的 `collectDiagnostics(cwd)`；为此把 `config.ts` 的解析段抽成 `parseConfigText(raw)` 并导出默认值常量，体检与启动因此共用**同一套** fail-closed 解析器（不会出现「体检说通过、启动却失败」）。文件缺失只报位置不建模板；语法错误返回 `errorLine`，此时 `vendors` 与 `global` 一律 `null`（不展示半截数据）；测试见 `inspect.test.ts`「只读体检」。
- [x] 8.2 实现逐供应商六项检查（必填字段 / endpoint / model / api_key 环境变量是否存在 / 价格完整性 / 探测配置）与待处理项计数，缺失项带下一步动作；验证方式是单测覆盖环境变量未设置、价格缺输出项、缺 endpoint、未配密钥四种情形。结论：六项检查返回结构化 `{ key, ok, detail, action }`，界面据此翻译标签；api_key **只回显变量名**（从原始 YAML 取未展开的 `api_key`，`describeApiKeySource` 对明文值只标注「不是环境变量占位符」绝不回显），并检查 `process.env` 是否已设置；`issueCount` 逐供应商与整体各计数一次；测试见 `inspect.test.ts`。
- [x] 8.3 实现全局与存储项（保留天数及来源、用量库路径与存在性及大小、探测默认值、会话 cwd），文件不存在时标注「不存在」；验证方式是单测覆盖用量库存在与不存在两种情况。结论：保留天数带「默认值 / 配置值」来源判定（看原始 YAML 有没有写 `retention.raw_records`）；用量库用 `defaultDatabasePath()` 定位，存在才给字节数、不存在为 `null`（界面写「不存在」而不是 0 字节）；探测默认值与 cwd 一并返回；测试见 `inspect.test.ts`「全局与存储项」。
- [x] 8.4 新增 `GET /api/diagnostics` 并要求凭据；验证方式是服务测试覆盖未授权 401 与正常返回。结论：路由独立成 `src/ui/routes/diagnostics.ts`（`DIAGNOSTICS_ROUTES`），`ApiContext` 新增 `cwd`，服务启动时由 `startDashboardServer(..., cwd)` 注入；接口只调 `collectDiagnostics(ctx.cwd)`，不写盘；测试见 `dashboard.test.ts`「体检接口」（含 401、快照字段与「查询前后配置文件内容不变」）。
- [x] 8.5 落地体检页 UI：配置明细、供应商体检表（密钥只显示变量名）、全局与存储、只读声明、解析失败替代卡（不展示半截供应商表）；验证方式是 HTML 测试断言解析失败时供应商表与全局项均不出现，并断言页面出现只读声明。结论：`settingsClientScript()` 渲染三块；解析失败时 `showSection(..., false)` 隐藏供应商表与全局项并显示替代卡（含错误、行号、路径与「不会自动修复」声明）；供应商名纵向合并、说明与动作列可换行；测试见 `dashboard-client.test.ts`「配置体检页脚本」与 `dashboard-html.test.ts`「配置体检页结构」。

## 9. 零数据引导态

- [x] 9.1 实现空态判定与引导页内容：未配供应商 / 有供应商但无记录两条分支，分别给出配置文件指引与产生数据的三条路径；验证方式是 HTML 测试覆盖两条分支，且返回主面板链接存在。结论：引导页复用 `/api/dashboard?period=30d` 判断（`vendors.length` 决定分支，`overview.requestCount` 决定文案），不新增后端接口；两条分支容器默认隐藏其一，由 `renderGuide()` 切换；已有记录时提示可以回主面板；测试见 `dashboard-client.test.ts`「零数据引导页脚本」与 `dashboard-html.test.ts`「零数据引导页结构」。
- [x] 9.2 让主面板在无数据时引导用户前往引导页，而不是渲染空表；验证方式是脚本测试断言无记录时出现引导入口。结论：概览的提示条与归因区块的空态都改为「文案 + 指向 `/empty` 的站内链接」（链接经 `wireNav()` 补凭据 fragment），主面板无记录时不再只留一张空表；断言见 `dashboard-client.test.ts`「本期没有记录时提示并给出去引导页的入口」与「归因无数据时显示暂无数据，不渲染空表」。

## 10. 中英双语

- [x] 10.1 为四个页面的可见文案建立 `data-i18n` 键与中英字典，切换时同步更新文本、`aria-label` 与 `<html lang>`；验证方式是脚本测试覆盖切换后文本与属性更新、刷新后语言保持。结论：字典是 `src/ui/client/messages.ts` 的单一事实来源（约 110 条），服务端用 `zh()` 输出默认中文并写 `data-i18n` / `data-i18n-aria`，页内 `i18nFragment()` 注入同一份 JSON 并实现 `t()` / `applyLanguage()`；语言偏好存于 localStorage 的 `kuma.lang`，`<head>` 的首帧脚本据此先写 `<html lang>`，正文由 body 末尾的片段同步替换（同一帧内完成，不闪现中文）。切换按钮是各页头部的 `#kuma-lang`，点击后先 `applyLanguage()` 再 `refresh()`，让表格、来源标记等脚本生成的文案一起换语言；`t()` 的内存语言优先于存储，隐私模式下也能本次会话生效。踩坑记录：页内脚本原有 `for (var t = 0; ...)` 会遮蔽取词函数 `t`，已把该循环变量改名为 `tick`。测试见 `dashboard-client.test.ts`「中英双语」与 `dashboard-html.test.ts`「中英双语结构」（含「字典覆盖服务端写出的每一个 data-i18n 键」）。
- [x] 10.2 人工核对四个页面在英文下的排版不溢出（按钮、表头、空态文案）；验证方式是在浏览器里逐页切换语言并确认无截断或换行错位。结论：用真实服务 + 无头 Chrome（agent-browser，CDP）逐页切换语言后测量 DOM，1280px 与 1000px 两种视口下四页均 `scrollWidth == clientWidth`（整页无横向溢出）、无 `scrollWidth > clientWidth` 的元素、无 `text-overflow: ellipsis` 截断、`[data-i18n]` 元素零中文残留；窄视口下 `.kuma-rail` 为 `display: none`。截图见会话记录（`/tmp/kuma-smoke/shots/en-*.png`）。**核对中发现并修掉三处真缺陷**：(1) 空态文案把中文全角括号写死在拼接里（`t("overview.emptyPrefix") + "（" + period + "）。"`、`t("common.noData") + "（" + period + "）"`），英文下出现 `No data（24h）`；现改为 `common.periodOpen/periodClose/sentenceEnd/separator/comma/colon` 六个随语言切换的标点词条，四处「连接失败」提示合并为 `failureStatus(reason)`。(2) 图表刻度与悬停详情用 `toLocaleString(undefined, ...)` 跟随系统 locale，英文界面显示 `20日 20:18`；现统一走 `dateLocale()`（en → `en-US`，zh → `zh-CN`），并把每个时间标签补上月份（`Sep 20, 11:21 AM`），在 918px 与 1198px 两种图宽下用 `getBBox()` 实测相邻标签零重叠。(3) `<title>` 只有中文，切英文后标签页仍显示中文；现把词条键写在 `<html data-title-key>` 上，`applyLanguage` 一并更新 `document.title`。三处都补了会失败的回归测试（含一次反证：把 `dateLocale()` 改回 `undefined` 时新测试确实变红）。已知边界（非缺陷）：体检页的诊断结论文案（如「已配置：https://...」「${VAR} 当前未设置」）按 `messages.ts` 的约定保持中文，因为它们含运行期值与变量名，双语化需要在服务端另出一套文案。

## 11. 收尾与验证

- [x] 11.1 更新 `README.md` 与 `README.zh-CN.md`：四个页面的进入方式、余额三档降级与手动填写位置、体检页只读边界、OAuth 的配置要求与不支持时的降级；验证方式是两份文档内容一致且不包含任何凭据示例明文。结论：两份 README 各新增一节「面板页面 / Dashboard pages」（四页路径表、页内链接经 URL fragment 带凭据、语言切换与 `kuma.lang`），并写清余额三档降级（接口 → OAuth → 手动，OAuth 四项配置缺一即视为未配置、令牌落盘位置与 `0600`、过期余量、手动填写写回位置与备份）、体检页的只读边界（不建模板、不修文件、只回显 `${ENV_VAR}` 变量名）、以及「三档全失败显示未知而不是 0、旧值保留并标注」；`Boundaries` 一节同步改为「只在明确触发时写入」，点明会写配置文件与令牌文件。两份文档标题与条目一一对应，示例只用 `${ENV_VAR}` 占位符，全篇无明文凭据。
- [x] 11.2 运行 `pnpm typecheck`、`pnpm -w run lint`、`pnpm test` 并全部通过；验证方式是三条命令退出码为 0 且无新增 lint error。结论：`pnpm typecheck`（tsc --noEmit）退出 0；`pnpm -w run lint` 退出 0、Found 0 error / 39 warnings（与本轮改动前的基线一致，均为 `useTopLevelRegex` 之类的既有性能提示）；`pnpm test` 18 个文件 293 条测试全部通过。
- [x] 11.3 运行 `pi -e ./src/index.ts` 冒烟：手工验证四个页面均可打开、主面板归因与概览随周期刷新、账户页同步按钮可用、体检页展示真实配置、Pi 退出后端口关闭；验证方式是逐项手工确认并记录结果。结论：`pi -e ./src/index.ts --no-session -p "..."` 能正常加载扩展并完成一次对话（无加载报错、无终端噪声）。用同一份 `startDashboardServer` 起真实服务 + 无头 Chrome 逐项核对：四页 200 且内容不含凭据（`tokenLeak=0`）、无凭据接口 401、错误 Host 403；主面板切 1h / 7d 后概览、归因、统计一起刷新且 `aria-pressed` 正确（`¥12.06 / 14 次` ↔ `¥0.30 / 1 次`）；账户页「同步全部」发出 `POST /api/accounts/sync` → 200 并刷新，手动值保留、未知不显示成 ¥0.00、日期显示为 `9/20/2026, 10:50:56 PM`；体检页展示真实 `config.yaml` 的逐供应商结论与存储路径。端口随会话关闭由 `index.test.ts`「session_shutdown(...) 释放端口并使旧凭据失效」覆盖（`startDashboardServer` 的 `close()` 即由该钩子调用）。**未在本机做的**：没有真实 TUI 会话可驱动（无 tmux / 无 pty），`/xpi-kuma` 命令本身的交互式触发由上述等价路径覆盖，若要在自己的 Pi 会话里验一遍，逐页打开与语言切换的截图证据在 `/tmp/kuma-smoke/shots/en-*.png`。
- [x] 11.4 用旧版本的 `usage.db` 副本做一次真实迁移冒烟，确认扩展启动后既有记录仍可查询、归因把历史记录归入「未知」；验证方式是查询结果包含迁移前后的记录且总花费不变。结论：用现网 `~/.pi/agent/data/xpi-kuma/usage.db`（旧 schema，无 `cwd` / `session_id` / `account_balances`）的 `sqlite3 .backup` 副本做真实迁移冒烟：迁移后补上两列并建出四张索引与 `account_balances` 表，**12619 条记录与总花费 166.1738 分毫不变**，四个时间范围下「统计请求数 = 项目维度 = 会话维度 = 供应商·模型维度」且花费三方相等（7d/30d 与全量一致），历史记录全部落入空键（界面显示「未知」）而不是被丢弃，连续打开两次不报错；原库保持旧 schema、未被本次冒烟改动。另有一份手工构造的旧 schema 迷你库做同样断言（`/tmp/kuma-migrate-real`、`/tmp/kuma-migrate-smoke`）。
