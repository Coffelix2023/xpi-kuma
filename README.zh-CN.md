...
| `xpi-kuma on [--port <port>] [--dev]` | 在任意终端启动独立后台服务；终端退出后服务继续运行。`--dev` 额外用浏览器打开带语义短码标签的调试页面（见下节） |
...
- `xpi-kuma on --dev` —— 同上，并在命令返回前用系统默认浏览器打开 `<URL>?semantic=1`，省得手动拼参数。
  服务已在运行时也生效：不重启服务，只再开一次调试页面。开浏览器失败只提示，不影响已启动的服务。
...
`.pi/prototype-design/kuma-dashboard/semantic-ui-map.yaml` 里的短码与全路径。

终端里 `xpi-kuma on --dev` 会启动（或复用）服务并直接用浏览器打开带开关的页面，不必手拼地址。手工打开
的话，任意面板 URL 加上 `?semantic=1` 即可：页面顶部居中会出现一枚胶囊（颜色随主题强对比），按下时徽标全开，
松开则拖动到任意位置；位置存 `localStorage`、开关状态存 `sessionStorage`，同标签页在四个页面之间
跳转时不必每次手动加参数。

开关读的是**进入页面那一刻**的 `?semantic=1`：页内脚本随后会把整个查询串从地址栏抹掉（清除凭据的
副作用），所以徽标脚本必须排在页内脚本之前执行；此后的刷新与站内跳转由 `sessionStorage` 兜底。
